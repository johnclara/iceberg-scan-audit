 ...
 */
public class AuditedManifestReader<F extends ContentFile<F>>
  ...
  // TODO added arg and changed visibility
  public AuditedManifestReader(String tableName, long scanId, InputFile file, Map<Integer, PartitionSpec> specsById,
                           InheritableMetadata inheritableMetadata, FileType content) {
    this.tableName = tableName;
    this.scanId = scanId;
   // TODO added arg and changed visibility END
    this.file = file;
  ...
  public AuditedManifestReader<F> select(Collection<String> newColumns) {
  ...
  public AuditedManifestReader<F> project(Schema newFileProjection) {
  ...
  public AuditedManifestReader<F> filterPartitions(Expression expr) {
    this.partFilter = Expressions.and(partFilter, expr);
    return this;
  }

  public AuditedManifestReader<F> filterRows(Expression expr) {
    this.rowFilter = Expressions.and(rowFilter, expr);
    return this;
  }

  public AuditedManifestReader<F> caseSensitive(boolean isCaseSensitive) {
  ...
  CloseableIterable<ManifestEntry<F>> entries() {
    if ((rowFilter != null && rowFilter != Expressions.alwaysTrue()) ||
      ...
      return CloseableIterable.filter(
          ...
          entry -> {
            if (entry != null) {
              boolean partitionEval = evaluator.eval(entry.file().partition());
              if (!partitionEval) {
                Listeners.notifyAll(ScanAuditEvaluateEvent.of(tableName, scanId, file.location(), entry.file().path().toString(), EvaluatorType.DATAFILE_PARTITION, partitionEval));
                return false;
              } else {
                boolean metricsEval = metricsEvaluator.eval(entry.file());
                Listeners.notifyAll(ScanAuditEvaluateEvent.of(tableName, scanId, file.location(), entry.file().path().toString(), EvaluatorType.DATAFILE_PARTITION, partitionEval));
                Listeners.notifyAll(ScanAuditEvaluateEvent.of(tableName, scanId, file.location(), entry.file().path().toString(), EvaluatorType.DATAFILE_METRICS, metricsEval));
                return partitionEval && metricsEval;
              }
            } else {
              return false;
            }
